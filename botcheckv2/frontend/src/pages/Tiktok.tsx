...
export default function Tiktok() {
  ...
  async function load() {
    try {
      ...
      setTracks(t);
      const v = await api("/api/video-tracks");
      setVideoTracks(v);
    } catch (e: any) {
  ...
  async function addTrack() {
    if (!username) return;
    try {
      await api("/api/tracks", { method: "POST", body: JSON.stringify({ tiktok_username: username }) });
  ...
  async function delTrack(tiktok_username: string) {
    if (!confirm("Xóa theo dõi tài khoản này?")) return;
    try {
      await api(`/api/tracks/${tiktok_username}`, { method: "DELETE" });
      load();
    } catch (e: any) {
      toast.error(e.message);
    }
  ...
  async function delVideo(video_id: string) {
    ...
    try {
      await api(`/api/video-tracks/${video_id}`, { method: "DELETE" });
      load();
    } catch (e: any) {
      toast.error(e.message);
    }
  ...
  async function updateVideoInterval(video_id: string) {
    try {
      await api(`/api/video-tracks/${video_id}`, { 
        method: "PUT", 
  ...
  return (
    <div className="flex flex-col gap-6 max-w-4xl">
      <h1 className="text-2xl font-semibold">Tiktok Tracking</h1>
        ...
        <CardContent className="flex flex-col gap-4">
          ...
          <div className="rounded-md border border-border">
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {tracks.map(t => (
                  <tr key={t.tiktok_username} className="border-b border-border last:border-0 hover:bg-muted/50">
                    ...
                    <td className="px-4 py-2">
                      <Button variant="ghost" size="sm" onClick={() => delTrack(t.tiktok_username)} className="text-die hover:text-die/80 h-8 px-2">
                        <IconTrash size={16} />
                      </Button>
                    </td>
                  </tr>
                  ...
                  <tr key={t.video_id} className="border-b border-border last:border-0 hover:bg-muted/50">
                    ...
                    <td className="px-4 py-2">
                      <div className="flex gap-3 text-xs">
                        <span>▶️ {t.last_plays?.toLocaleString()}</span>
