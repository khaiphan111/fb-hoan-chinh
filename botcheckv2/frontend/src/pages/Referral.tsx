      ...
      <Card>
        ...
        <CardContent>
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {data.withdraw_requests?.map((r: any) => (
                <tr key={r.id} className="border-b">
                  <td className="p-2">{r.user_id}</td>
                  <td className="p-2 text-green-500">{new Intl.NumberFormat("vi-VN").format(r.amount)}đ</td>
              ...
              {!data.withdraw_requests?.length && (
                <tr>
                  <td colSpan={4} className="p-4 text-center text-muted-foreground">Không có yêu cầu nào</td>
                </tr>
              )}
            </tbody>
          </table>
